...
# Setup logging
def setup_logging():
    # Create logs/v2 directory if it doesn't exist
    if not os.path.exists('logs/v2'):
        os.makedirs('logs/v2')
    
    # Create a unique log file name with timestamp and target crop
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    log_file = f'logs/v2/hybrid_laplacian_crop{TARGET_CROP}_{timestamp}.log'
...
def validate_model(features_extractor, data_loader, positive_center, negative_center, alpha, d_star, order, edges, edge_i, edge_j, logger, dataset_name="Validation"):
        ...
        for bands, label in tqdm(data_loader, desc=f"{dataset_name}"):
            ...
            for i in range(2):
                for j in range(2):
                    ...
                    degree = np.abs(sparse_adjacency).sum(axis=1).A1
                    D = diags(degree)
                    L_signed = D - sparse_adjacency

                    L = alpha * L_combinatorial + (1 - alpha) * L_signed
...
def find_best_alpha(features_extractor, val_loader, positive_center, negative_center, d_star, order, edges, edge_i, edge_j, logger):
    """Find the best alpha value based on validation performance"""
    logger.info("=" * 50)
    logger.info("FINDING BEST ALPHA VALUE")
    logger.info("=" * 50)
    ...
    for alpha in ALPHA_VALUES:
        ...
        val_accuracy, val_f1 = validate_model(
            ...
            alpha, d_star, order, edges, edge_i, edge_j, logger, f"Validation (alpha={alpha})"
        )
    ...
    logger.info("\n" + "=" * 50)
    logger.info("ALPHA SELECTION RESULTS")
    logger.info("=" * 50)
    for alpha in ALPHA_VALUES:
        results = alpha_results[alpha]
        logger.info(f"Alpha {alpha}: Accuracy = {results['accuracy']:.4f}, F1 = {results['f1']:.4f}")
    
    logger.info(f"\nBest alpha: {best_alpha} with F1 score: {best_f1:.4f}")
...
def run_validation():
    ...
    # Find best alpha
    d_star = 1.0
    best_alpha, alpha_results = find_best_alpha(
        features_extractor, val_loader, positive_center, negative_center,
        d_star, order, edges, edge_i, edge_j, logger
    )
    
    # Run validation on test set with best alpha
    logger.info("\n" + "=" * 50)
    logger.info(f"TEST SET VALIDATION WITH BEST ALPHA ({best_alpha})")
    logger.info("=" * 50)
    
    test_accuracy, test_f1 = validate_model(
        features_extractor, test_loader, positive_center, negative_center,
        best_alpha, d_star, order, edges, edge_i, edge_j, logger, "Test"
    )
    
    logger.info(f"\nFinal Test Results with alpha={best_alpha}:")
