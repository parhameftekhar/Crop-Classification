...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'logs/benchmark/multi_case/training_dpt_multiclass_{NUM_CLASSES}classes.log'),
...
# TensorBoard writer
writer = SummaryWriter(log_dir=f'tensorboard/benchmark/multi_case/dpt')

# Model setup
logger.info(f'Initializing DPT model for {NUM_CLASSES}-class classification')
...
# Modify the first layer to accept 18 input channels
original_conv = model.dpt.embeddings.patch_embeddings.projection
...
model.dpt.embeddings.patch_embeddings.projection = new_conv
# Patch all internal num_channels attributes — these are set at __init__ time
# from config.num_channels and are NOT read dynamically, so they must be updated manually
model.config.num_channels = 18
model.dpt.embeddings.patch_embeddings.num_channels = 18  # DPTPatchEmbeddings check (line ~290)
model.dpt.embeddings.num_channels = 18                  # DPTViTEmbeddings check (line ~169)
...
train_loader = setup_multiclass_loader(
    ...
    batch_size=4,  # Reduced batch size for DPT
    crop_band_index=18,
    device='cuda'
)
...
def train_epoch(model, train_loader, criterion, optimizer, device, num_classes):
    ...
    for images, labels in pbar:
        # DPT expects normalization to [0,1] or standard normalization, but we'll feed raw values
        # permute to (B, C, H, W)
        images = images.permute(0, 3, 1, 2).to(device)
        labels = labels.to(device)
        
        # DPT forward pass
        # Hugging Face models expect 'pixel_values'
        outputs = model(pixel_values=images)
        logits = outputs.logits
        
        # Upsample logits to match label size if necessary
        ...
        accumulate_confusion_matrix(logits, labels, num_classes, confusion_matrix)
...
for epoch in epoch_pbar:
    ...
    if val_miou > best_val_miou:
        ...
        torch.save(model.state_dict(), f'checkpoints/benchmark/multi_case/best_dpt_model_multiclass_{NUM_CLASSES}classes.pth')
        logger.info(f'Epoch {epoch+1}/{num_epochs} - New best model saved with validation mIoU: {val_miou:.4f}')

# Load best model for testing
model.load_state_dict(torch.load(f'checkpoints/benchmark/multi_case/best_dpt_model_multiclass_{NUM_CLASSES}classes.pth'))
