...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'logs/benchmark/multi_case/training_segformer_multiclass_{NUM_CLASSES}classes.log'),
...
# TensorBoard writer
writer = SummaryWriter(log_dir=f'tensorboard/benchmark/multi_case/segformer')

# Model setup
logger.info(f'Initializing SegFormer model for {NUM_CLASSES}-class classification')

# Create configuration from scratch or load pretrained
# We'll use a config similar to mit-b0 but adapted for our input
config = SegformerConfig(
    num_labels=NUM_CLASSES,
    image_size=224,  # Assuming input size
...
# Ensure the first layer has correct channels (it should be set by config, but double checking)
if model.segformer.encoder.patch_embeddings[0].proj.in_channels != 18:
    logger.info("Modifying input layer to accept 18 channels...")
...
def train_epoch(model, train_loader, criterion, optimizer, device, num_classes):
    ...
    for images, labels in pbar:
        ...
        # Upsample logits to match label size (SegFormer outputs 1/4 res by default)
...
for epoch in epoch_pbar:
    ...
    if val_miou > best_val_miou:
        ...
        torch.save(model.state_dict(), f'checkpoints/benchmark/multi_case/best_segformer_model_multiclass_{NUM_CLASSES}classes.pth')
        logger.info(f'Epoch {epoch+1}/{num_epochs} - New best model saved with validation mIoU: {val_miou:.4f}')

# Load best model for testing
model.load_state_dict(torch.load(f'checkpoints/benchmark/multi_case/best_segformer_model_multiclass_{NUM_CLASSES}classes.pth'))
