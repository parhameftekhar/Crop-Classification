...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'logs/benchmark/multi_case/training_pspnet_multiclass_{NUM_CLASSES}classes.log'),
...
# TensorBoard writer
writer = SummaryWriter(log_dir=f'tensorboard/benchmark/multi_case/pspnet')

# Model setup
logger.info(f'Initializing PSPNet model for {NUM_CLASSES}-class classification')
model = smp.PSPNet(
    encoder_name="resnet50",        # Use ResNet50 as encoder
    encoder_weights="imagenet",     # Use ImageNet pretrained weights
    in_channels=18,                 # 18 input channels for satellite data
    classes=NUM_CLASSES,            # Multi-class classification
    activation=None,                # No activation, we'll use CrossEntropyLoss
    encoder_depth=3,                # Standard depth for ResNet50
    psp_use_batchnorm=True,         # Use batch normalization in PSP module
    psp_dropout=0.2,                # Dropout in PSP module
    psp_conv_filters=256,           # Number of filters in PSP convolutions
    aux_params=None,                # No auxiliary loss
    upsampling=8                    # Upsampling factor to match input size (224x224)
)
...
for epoch in epoch_pbar:
    ...
    if val_miou > best_val_miou:
        ...
        torch.save(model.state_dict(), f'checkpoints/benchmark/multi_case/best_pspnet_model_multiclass_{NUM_CLASSES}classes.pth')
        logger.info(f'Epoch {epoch+1}/{num_epochs} - New best model saved with validation mIoU: {val_miou:.4f}')

# Load best model for testing
model.load_state_dict(torch.load(f'checkpoints/benchmark/multi_case/best_pspnet_model_multiclass_{NUM_CLASSES}classes.pth'))
