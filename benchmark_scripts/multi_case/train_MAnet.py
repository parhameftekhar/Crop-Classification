...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'logs/benchmark/multi_case/training_manet_multiclass_{NUM_CLASSES}classes.log'),
...
# TensorBoard writer
writer = SummaryWriter(log_dir=f'tensorboard/benchmark/multi_case/manet')

# Model setup
logger.info(f'Initializing MAnet model for {NUM_CLASSES}-class classification')
model = smp.MAnet(
    ...
    decoder_pab_channels=64,  # Position Attention Block channels
)
...
train_loader = setup_multiclass_loader(
    ...
    batch_size=8,  # Reduced batch size for MAnet (larger model)
    crop_band_index=18,
    device='cuda'
)
...
for epoch in epoch_pbar:
    ...
    if val_miou > best_val_miou:
        ...
        torch.save(model.state_dict(), f'checkpoints/benchmark/multi_case/best_manet_model_multiclass_{NUM_CLASSES}classes.pth')
        logger.info(f'Epoch {epoch+1}/{num_epochs} - New best model saved with validation mIoU: {val_miou:.4f}')

# Load best model for testing
model.load_state_dict(torch.load(f'checkpoints/benchmark/multi_case/best_manet_model_multiclass_{NUM_CLASSES}classes.pth'))
