...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'logs/benchmark/binary_case/training_dpt_hf_binary_crop{TARGET_CROP}.log'),
...
# Model setup
logger.info('Initializing DPT model from Hugging Face for binary classification')
model = DPTForSemanticSegmentation.from_pretrained(
    "Intel/dpt-large-ade",  # You can also use "Intel/dpt-base-ade" or "Intel/dpt-hybrid-ade"
...
train_loader = setup_training_loader(
    ...
    train_batch_size=4,  # Reduced batch size for DPT as it's larger
    crop_band_index=18,
    device='cuda',
    ignore_crops=None,
    min_ratio=0,
...
for epoch in epoch_pbar:
    ...
    if val_f1 > best_val_f1:
        best_val_f1 = val_f1
        torch.save(model.state_dict(), f'checkpoints/benchmark/binary_case/best_dpt_hf_model_binary_crop{TARGET_CROP}.pth')
...
model.load_state_dict(torch.load(f'checkpoints/benchmark/binary_case/best_dpt_hf_model_binary_crop{TARGET_CROP}.pth'))
