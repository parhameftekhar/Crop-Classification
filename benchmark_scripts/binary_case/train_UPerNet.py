...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'logs/benchmark/binary_case/training_upernet_binary_crop{TARGET_CROP}.log'),
...
# Model setup
logger.info('Initializing UPerNet model for binary classification')
model = smp.UPerNet(
...
train_loader = setup_training_loader(
    ...
    train_batch_size=8,  # Reduced batch size for UPerNet as it's larger than UNet
    crop_band_index=18,
    device='cuda',
    ignore_crops=None,
    min_ratio=0,
...
for epoch in epoch_pbar:
    ...
    if val_f1 > best_val_f1:
        best_val_f1 = val_f1
        torch.save(model.state_dict(), f'checkpoints/benchmark/binary_case/best_upernet_model_binary_crop{TARGET_CROP}.pth')
...
model.load_state_dict(torch.load(f'checkpoints/benchmark/binary_case/best_upernet_model_binary_crop{TARGET_CROP}.pth'))
