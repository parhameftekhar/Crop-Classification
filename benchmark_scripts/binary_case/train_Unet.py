...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'logs/benchmark/binary_case/training_unet_binary_crop{TARGET_CROP}.log'),
...
# Model setup
logger.info('Initializing UNet model for binary classification')
model = smp.Unet(
...
def train_epoch(model, train_loader, criterion, optimizer, device):
    ...
    for images, labels in pbar:
        ...
        # UNet returns logits directly (not in a dictionary like FCN)
...
for epoch in epoch_pbar:
    ...
    if val_f1 > best_val_f1:
        best_val_f1 = val_f1
        torch.save(model.state_dict(), f'checkpoints/benchmark/binary_case/best_unet_model_binary_crop{TARGET_CROP}.pth')
...
model.load_state_dict(torch.load(f'checkpoints/benchmark/binary_case/best_unet_model_binary_crop{TARGET_CROP}.pth'))
