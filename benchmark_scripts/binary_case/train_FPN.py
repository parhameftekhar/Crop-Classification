...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'logs/benchmark/binary_case/training_fpn_binary_crop{TARGET_CROP}.log'),
...
# Model setup
logger.info('Initializing FPN model for binary classification')
model = smp.FPN(
    ...
    pyramid_channels=256,  # Number of channels in pyramid blocks
    segmentation_channels=128,  # Number of channels in segmentation head
...
for epoch in epoch_pbar:
    ...
    if val_f1 > best_val_f1:
        best_val_f1 = val_f1
        torch.save(model.state_dict(), f'checkpoints/benchmark/binary_case/best_fpn_model_binary_crop{TARGET_CROP}.pth')
...
model.load_state_dict(torch.load(f'checkpoints/benchmark/binary_case/best_fpn_model_binary_crop{TARGET_CROP}.pth'))
